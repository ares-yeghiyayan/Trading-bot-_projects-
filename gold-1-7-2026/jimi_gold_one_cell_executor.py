# ============================================================
# JIMI GOLD ONE-CELL EXECUTOR
# For Jupyter Notebook + MetaTrader 5
# Gold/XAUUSD only. Forex is blocked.
# ============================================================
...
JIMI_COMMANDS = """
[ XAUUSD = 3998.00 short | SL=4022.00 | TP=3948.00 | LOT=0.01 | EXP=0 | COMMENT=Jimi_Gold_SellPullback ]
"""
...
MAGIC_NUMBER = 26063099
MANAGED_MAGIC_NUMBERS = [26063099]

# اگر بروکرت اسم طلا را متفاوت گذاشته باشد اینجا اضافه کن
GOLD_SYMBOLS_ALLOWED = [
    "XAUUSD",
    "XAUUSDm",
    "XAUUSD.",
    "GOLD",
...
# کنترل اسپرد طلا با point
# مثلا اگر point = 0.01 باشد، 150 points یعنی حدود 1.50 دلار
MAX_SPREAD_POINTS = 180

# حداقل فاصله SL/TP از entry یا market price
# اگر point = 0.01 باشد، 300 points یعنی حدود 3 دلار
MIN_SL_DISTANCE_POINTS = 300
MIN_TP_DISTANCE_POINTS = 300
...
def is_gold_symbol_name(symbol):
    s = symbol.upper()
    return "XAU" in s or "GOLD" in s


def resolve_gold_symbol(symbol):
    """
    اگر کاربر XAUUSD بنویسد ولی بروکر XAUUSDm داشته باشد،
    سعی می‌کند نماد درست را پیدا کند.
    """
    raw = symbol.strip()

    if not is_gold_symbol_name(raw):
        raise ValueError(f"{raw}: blocked. This script is gold only.")

    candidates = []

    # اول همان چیزی که کاربر نوشته
    candidates.append(raw)

    # بعد نسخه‌های مجاز
    for s in GOLD_SYMBOLS_ALLOWED:
    ...
    raise ValueError(
        f"{raw}: gold symbol not found in MT5. "
        f"Allowed aliases: {GOLD_SYMBOLS_ALLOWED}. "
        "Check your broker symbol name."
    )
...
def is_allowed_gold_runtime_symbol(symbol):
    return is_gold_symbol_name(symbol)
...
def parse_jimi_commands(text):
    ...
    for raw_line in text.strip().splitlines():
        ...
        symbol = resolve_gold_symbol(symbol_raw)
...
def validate_order(order):
    ...
    if not is_allowed_gold_runtime_symbol(symbol):
        return False, f"{symbol}: blocked. This script is gold only."
    ...
    if spread_points > MAX_SPREAD_POINTS:
        return False, f"{symbol}: spread too high: {spread_points:.1f} points | max={MAX_SPREAD_POINTS}"
    ...
    sl_distance_points = abs(ref_price - sl) / info.point
    tp_distance_points = abs(ref_price - tp) / info.point

    if sl_distance_points < MIN_SL_DISTANCE_POINTS:
        return False, f"{symbol}: SL too close. Distance={sl_distance_points:.1f} points"

    if tp_distance_points < MIN_TP_DISTANCE_POINTS:
        return False, f"{symbol}: TP too close. Distance={tp_distance_points:.1f} points"
    ...
    return True, (
        ...
        f"Bid={tick.bid} Ask={tick.ask} Spread={spread_points:.1f} points"
    )
...
def close_position(position):
    ...
    request = {
        ...
        "deviation": DEVIATION_POINTS,
        "magic": MAGIC_NUMBER,
        "comment": "Jimi_close_gold_opp",
        "type_time": mt5.ORDER_TIME_GTC,
    }

    return send_order_with_fallback(request)
...
def reconcile_old_signals(new_orders):
    ...
    for new_order in new_orders:
        ...
        # 1) Delete old pending orders for same gold symbol
        ...
        # 2) Close opposite active gold positions
...
if len(orders) == 0:
    raise RuntimeError("No gold orders found in JIMI_COMMANDS.")

if len(orders) > MAX_ORDERS_PER_RUN:
    raise RuntimeError(f"Too many gold orders. Max allowed: {MAX_ORDERS_PER_RUN}")

print("\nParsed Gold Commands:")
...
if not all(row["valid"] for row in validation_rows):
    raise RuntimeError("Validation failed. Fix the blocked gold order before execution.")

# ============================================================
# 13) Reconcile old signals
# ============================================================

if RECONCILE_OLD_SIGNALS:
    print("\nReconciling Old Gold Signals:")
    reconciliation = reconcile_old_signals(orders)

    if reconciliation:
        display(pd.DataFrame(reconciliation))
    else:
        print("No old opposite/same-symbol Jimi gold orders found.")
...
for o in orders:
    ...
    if same_side_exists and BLOCK_NEW_ORDER_IF_SAME_SIDE_POSITION_EXISTS:
        results.append({
            ...
            "message": (
                f"Same-side active Jimi gold position already exists. "
...
# ============================================================
# 15) Show current Jimi gold pending orders and positions
# ============================================================

print("\nCurrent Jimi Gold Pending Orders:")
...
if all_pending:
    for p in all_pending:
        if is_allowed_gold_runtime_symbol(p.symbol) and is_jimi_managed(p):
            pending_rows.append({
                "ticket": p.ticket,
                "symbol": p.symbol,
...
print("\nCurrent Jimi Gold Active Positions:")
